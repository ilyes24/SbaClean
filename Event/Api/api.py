...
class EventParticipationAPIView(mixins.CreateModelMixin, generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if query_post is not None:
            qs = qs.filter(Q(event__post__exact=query_post)).distinct()

        if query_date is not None:
            qs = qs.filter(Q(event__starts_at__lte=query_date)).distinct()

        if query_owner is not None:
            qs = qs.filter(Q(event__post__post_owner__exact=query_owner)).distinct()

        if query_title is not None:
            qs = qs.filter(Q(event__post__title__exact=query_title)).distinct()

        if query_city is not None:
            qs = qs.filter(Q(event__post__city__exact=query_city)).distinct()

        if query_description is not None:
            qs = qs.filter(Q(event__post__description__contains=query_description)).distinct()
