...
def feed(request):
    ...
    if query_limit is not None:
        ...
        posts2 = posts2.annotate(rank_point=(Count('post__reactions', filter=Q(post__reactions__is_like=True)))).filter(
            rank_point__gt=0)
    ...
    if request.method == 'POST':
        ...
        if form2.is_valid():
            ...
            description = request.POST.get('description')
            post = Post.objects.create(title=title, post_owner=request.user, description=description,
                                       city=get_object_or_404(City, id=city), longitude=longitude, latitude=latitude)
            post.save()
        ...
        form = UserComment()
        form2 = UserPost()
    context = {'username': username, 'user_pic': user_pic, 'users': users, 'posts': posts, 'userId': userId,
               'comments': comments, 'form': form, 'form2': form2, 'reactions': reactions,
...
def event(request):
    ...
    if request.method == 'POST':
        ...
        form = UserComment()
        form2 = UserPost()
    context = {'username': username, 'user_pic': user_pic, 'events': events, 'userId': userId, 'users': users,
               'comments': comments, 'form': form, 'form2': form2, 'reactions': reactions,
...
def Myposts(request):
    ...
    if request.method == 'POST':
        ...
        form = UserComment()
        form2 = UserPost()
    context = {'username': username, 'user_pic': user_pic, 'posts': posts, 'userId': userId, 'users': users,
               'comments': comments, 'form': form, 'form2': form2, 'reactions': reactions,
               'like_creat_nember': like_creat_nember,
...
def Myreactions(request):
    ...
    context = {'username': username, 'user_pic': user_pic, 'posts': posts, 'userId': userId, 'users': users,
               'comments': comments, 'form': form, 'form2': form2, 'reactions': reactions,
               'like_creat_nember': like_creat_nember,
               'posts2': posts2, 'Myreactions': Myreactions, 'Mycomments': Mycomments,
               'valide_comments': valide_comments}
    ...
    if request.method == 'POST':
        ...
        if request.GET.get('partial_token'):
            ...
            form = UserRegistreForm(
                initial={'username': data['username'], 'email': data['email'], 'First_name': data['first_name'],
